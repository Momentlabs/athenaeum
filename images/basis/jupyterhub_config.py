...
if auth_type == 'google':
    ...
    c.Auth0Authenticator.authenticator_class = 'oauthenticator.auth0.Auth0Authenticator'
    c.Auth0Authenticator.client_id = get_config('auth.auth0.client-id')
    c.Auth0Authenticator.client_secret = get_config('auth.auth0.client-secret')
    c.Auth0Authenticator.oauth_callback_url = get_config('auth.auth0.callback-url')
...
if not cloud_metadata.get('enabled', False):
    ...
    ip_block_container = client.V1Container(
        ...
        image="{0}:{1}".format(network_tools_image_name,network_tools_image_tag),
        command=[
            'iptables',
            '-A', 'OUTPUT',
